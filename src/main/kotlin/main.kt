...
suspend fun main() {
    ...
    val episodesNotYetDownloaded = episodes.filter { !it.htmlsDownloaded }
...
fun downloadEpisode(client: OkHttpClient, episode: Episode) {
    ...
    val request = Request.Builder()
        ...
        .url("https://www.newsinslowitalian.com/series/episode/${episode.episodeNumber}")
        .build()
...
fun createMp3File(episode: Episode, byteStream: InputStream) {
    val file = File("episodes/htmls/${episode.episodeNumber}-${episode.year}.html")
    file.createNewFile()
